...
class StreamsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create stream" do
    assert_difference('Stream.count') do
      post streams_url, params: { stream: { collection: @stream.collection, date: @stream.date } }
    end
  ...
  test "should update stream" do
    patch stream_url(@stream), params: { stream: { collection: @stream.collection, date: @stream.date } }
